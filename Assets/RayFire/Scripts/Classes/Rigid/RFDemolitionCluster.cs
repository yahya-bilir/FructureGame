...
namespace RayFire
{
    ...
    public class RFDemolitionCluster
    {
        ...
        static Collider[] GetDetachColliders (RayfireRigid scr)
        {
            ...
            {
                Collider[] colliders = Physics.OverlapSphere (scr.lim.contactVector3, scr.clsDemol.damageRadius, 1 << scr.gameObject.layer);
                scr.clsDemol.damageRadius = 0f;
                return colliders;
            }
            ...
            if (scr.clsDemol.type == RFDetachType.WorldUnits)
                if (scr.clsDemol.units > 0)
                    return Physics.OverlapSphere (scr.lim.contactVector3, scr.clsDemol.units, 1 << scr.gameObject.layer);
            ...
            float contactRadius = scr.lim.bboxSize / 100f * scr.clsDemol.ratio;
            return Physics.OverlapSphere (scr.lim.contactVector3, contactRadius, 1 << scr.gameObject.layer);
        }
        ...
        public static void PostDemolitionCluster (RayfireRigid scr, List<RFShard> detachShards)
        {
            ...
            // Init sound
            RFSound.DemolitionSound(scr.sound, scr.lim.bboxSize);
        ...
        static void UpdateOriginalClusterOLD (RayfireRigid scr)
        {
            ...
            if (scr.simTp == SimType.Kinematic || scr.simTp == SimType.Inactive)
                if(scr.act.con == true && scr.clsDemol.cluster.UnyieldingByShard == false)
                    scr.Activate();
        ...
        public static void SliceConnectedCluster (RayfireRigid scr)
        {
            ...
            if (scr.clsDemol.type == RFDetachType.RatioToSize)
                detachDistance = scr.lim.bboxSize / 100f * scr.clsDemol.ratio;
            ...
            if (detachShards.Count > 0)
            {
                ...
                scr.lim.contactVector3 = plane.ClosestPointOnPlane (detachShards[0].tm.position);
                scr.lim.contactNormal  = plane.normal;
        ...
        static void ClusterizeDetachShards (RayfireRigid scr, List<RFShard> detachShards, int clusterAmount, int sortType)
        {
            ...
            if (centerShardsAmount > 0)
            {
                // Get center shards
                if (sortType == 0)
                    center = RFShard.SortByDistanceToPoint (detachShards.ToArray(), scr.lim.contactVector3, centerShardsAmount);
                else if (sortType == 1)
                    center = RFShard.SortByDistanceToPlane (detachShards.ToArray(), scr.lim.contactVector3, scr.lim.contactNormal, centerShardsAmount);
                if (center != null)
                {
        ...
        public static void CreateClusterRuntime (RayfireRigid scr, RFCluster cluster)
        {
            ...
            if (cluster.rigid.simTp == SimType.Kinematic || cluster.rigid.simTp == SimType.Inactive)
                if (cluster.rigid.act.con == true && cluster.UnyieldingByShard == false)
            ...
            {
                // Copy depth
                cluster.rigid.lim.depth = scr.lim.depth;
                
                // Set current depth
                cluster.rigid.lim.currentDepth = scr.lim.currentDepth + 1;
                
                // last demolition depth
                if (scr.lim.depth != 0 && cluster.rigid.lim.currentDepth >= scr.lim.depth)
        ...
        static void AddRigidComponent (RayfireRigid scr, RFShard shard, Transform parent)
        {
            ...
            // Set unyielding
            shard.rigid.act.uny = shard.uny;
            shard.rigid.act.atb = shard.act;
            ...
            if (shard.rigid.simTp == SimType.Kinematic || shard.rigid.simTp == SimType.Inactive)
                if (shard.rigid.act.con == true && shard.uny == false)
                {
