...
namespace RayFire
{
    ...
    public class RFCollapse
    {
        ...
        public static void DamageCollapse(RayfireRigid scr, float value)
        {
            ...
            // Already collapsing
            if (scr.act.cnt.collapse.inProgress == true)
                return;
                
            // Damage collapse disabled
            if (scr.act.cnt.startCollapse != RayfireConnectivity.RFConnInitType.ByDamage)
                return;
            
            // Add damage
            scr.act.cnt.collapseByDamageCur += value;
            
            // Start collapse if max reached
            if (scr.act.cnt.collapseByDamageCur >= scr.act.cnt.collapseByDamageMax)
                StartCollapse (scr.act.cnt);
        }
