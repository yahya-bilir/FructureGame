...
namespace RayFireEditor
{
    ...
    public class RayfireBombEditor : Editor
    {
        ...
        void GUI_Actions()
        {
            ...
            {
                RFUI.Caption (TextBmb.gui_cap_actions);
                ...
                if (GUILayout.Button (TextBmb.gui_btn_explode, GUILayout.Height (25)))
                {
                    foreach (RayfireBomb script in targets)
                    {
                        ...
                        RFUI.SetDirty (script.gameObject);
                    }
                }
        ...
        void GUI_Range()
        {
            RFUI.CaptionBox (TextBmb.gui_cap_range);
            RFUI.PropertyField (sp_showGizmo, TextBmb.gui_showGizmo);
            RFUI.PropertyField (sp_rangeType, TextBmb.gui_rangeType);
            RFUI.PropertyField (sp_range,     TextBmb.gui_range);

            if (bomb.rangeType == RayfireBomb.RangeType.Directional)
                RFUI.PropertyField (sp_boxSize, TextBmb.gui_boxSize);

            RFUI.IntSlider (sp_deletion, deletion_min, deletion_max, TextBmb.gui_deletion);
        }
        ...
        void GUI_Impulse()
        {
            RFUI.CaptionBox (TextBmb.gui_cap_impulse);
            RFUI.PropertyField (sp_fadeType, TextBmb.gui_fadeType);
            
            if (bomb.fadeType == RayfireBomb.BombFadeType.ByCurve)
            {
                EditorGUILayout.CurveField (sp_curve, RFUI.color_orange, new Rect (new Vector2 (0, 0), new Vector2 (1, 1)), TextBmb.gui_curve);
                RFUI.Space ();
            }
            
            RFUI.PropertyField (sp_strength, TextBmb.gui_strength);
            RFUI.IntSlider (sp_variation, variation_min, variation_max, TextBmb.gui_variation);
            RFUI.IntSlider (sp_chaos,     chaos_min,     chaos_max,     TextBmb.gui_chaos);
            RFUI.PropertyField (sp_forceByMass, TextBmb.gui_forceByMass);
        }
        ...
        void GUI_Activation()
        {
            RFUI.CaptionBox (TextBmb.gui_cap_act);
            RFUI.PropertyField (sp_affectInactive,  TextBmb.gui_affectInactive);
            RFUI.PropertyField (sp_affectKinematic, TextBmb.gui_affectKinematic);
        }
        ...
        void GUI_Damage()
        {
            RFUI.CaptionBox (TextBmb.gui_cap_dmg);
            RFUI.PropertyField (sp_applyDamage, TextBmb.gui_applyDamage);
            if (sp_applyDamage.boolValue == true)
                RFUI.Slider (sp_damageValue, damageValue_min, damageValue_max, TextBmb.gui_damageValue);
        }
        ...
        void GUI_Detonation()
        {
            RFUI.CaptionBox (TextBmb.gui_cap_prop);
            RFUI.Foldout (ref fld_det, TextKeys.bmb_fld_det, TextBmb.gui_fld_det.text);
            if (fld_det == true)
            {
        ...
        void GUI_Obstacles()
        {
            RFUI.Foldout (ref fld_obs, TextKeys.bmb_fld_obs, TextBmb.gui_fld_obs.text);
            if (fld_obs == true)
            {
                EditorGUI.indentLevel++;
                RFUI.PropertyField (sp_obst_enable,    TextBmb.gui_obst_enable);
                RFUI.PropertyField (sp_obst_static,    TextBmb.gui_obst_static);
                RFUI.PropertyField (sp_obst_kinematik, TextBmb.gui_obst_kinematik);
        ...
        void GUI_Audio()
        {
            RFUI.Foldout (ref fld_aud, TextKeys.bmb_fld_aud, TextBmb.gui_fld_aud.text);
            if (fld_aud == true)
            {
                EditorGUI.indentLevel++;
                RFUI.PropertyField (sp_play, TextBmb.gui_play);
                RFUI.Slider (sp_volume, volume_min, volume_max, TextBmb.gui_volume);
        ...
        void GUI_Filters()
        {
            RFUI.Foldout (ref fld_fil, TextKeys.bmb_fld_fil, TextBmb.gui_fld_fil.text);
            if (fld_fil == true)
            {
                EditorGUI.indentLevel++;
                RFUI.TagField (sp_tag, TextBmb.gui_tagFilter);
                RFUI.MaskField (sp_mask, TextBmb.gui_mask);
                EditorGUI.indentLevel--;
            }
        }
