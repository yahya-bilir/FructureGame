...
{
    [SelectionBase]
    [AddComponentMenu (RFLog.dbr_path)]
    [HelpURL (RFLog.dbr_link)]
    public class RayfireDebris : MonoBehaviour
    {
        ...
        void SetReferenceMeshes(GameObject refs)
        {
            ...
            if (refs == null)
            {
                RayfireMan.Log (RFLog.dbr_dbgn + name + RFLog.dbr_noRef, gameObject);
                return;
            }
            ...
            if (stMfs.Length == 0)
            {
                RayfireMan.Log (RFLog.dbr_dbgn + name + RFLog.dbr_noMesh, gameObject);
                return;
            }
            ...
            for (int i = 0; i < stMfs.Length; i++)
            {
                ...
                if (stMfs[i].sharedMesh != null && stMfs[i].sharedMesh.vertexCount > 3)
                    ...
                    RayfireMan.Log (RFLog.dbr_dbgn + name + RFLog.dbr_noVert, gameObject);
            }
            
            // Check for deprecated burst property. Will be OBSOLETE
            if ((int)emission.burstType == 1)
                RayfireMan.Log (RFLog.dbr_dbgn + name + RFLog.dbr_noBurs, gameObject);
