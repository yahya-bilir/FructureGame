...
namespace RayFire
{
    ...
    public class RayfireUnyielding : MonoBehaviour
    {
        ...
        static void SetRigidUnyState (RayfireUnyielding uny)
        {
            ...
            {
                uny.rigidList[i].act.uny = uny.unyielding;
                uny.rigidList[i].act.atb = uny.activatable;
            }
        ...
        public static void SetUnyieldingFragments (RayfireRigid scr, bool slice)
        {
            ...
            if (scr.mshDemol.prp.l == false)
                layerMask = 1 << scr.mshDemol.prp.lay;
        ...
        public static void SetUnyielding(RayfireRigidRoot rr)
        {
            ...
            {
                rr.meshRootShards[i].uny = rr.meshRootShards[i].rigid.act.uny;
                rr.meshRootShards[i].act = rr.meshRootShards[i].rigid.act.atb;
            }
            
            // Set by connected cluster
            for (int i = 0; i < rr.connClusterShards.Count; i++)
            {
                rr.connClusterShards[i].uny = rr.connClusterShards[i].rigid.act.uny;
                rr.connClusterShards[i].act = rr.connClusterShards[i].rigid.act.atb;
            }
        ...
        public void Activate()
        {
            ...
            if (HasRigids == true)
            {
                for (int i = 0; i < rigidList.Count; i++)
                {
                    ...
                    if (rigidList[i].act.atb == true)
                    {
                        rigidList[i].Activate (i == rigidList.Count - 1);
                        rigidList[i].act.cnt = null;
                    }
                }
            }
