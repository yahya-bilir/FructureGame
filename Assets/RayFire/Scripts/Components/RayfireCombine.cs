...
namespace RayFire
{
    [AddComponentMenu(RFLog.cmb_path)]
    [HelpURL(RFLog.cmb_link)]
    public class RayfireCombine : MonoBehaviour
    {
        ...
        bool SetData()
        {
            ...
            if (meshList.Count == 0)
            {
                RayfireMan.Log (RFLog.cmb_dbgn + name + RFLog.cmb_noMesh, gameObject);
                return false;
            }

            // Vert limit reached
            if (totalVerts >= 65535)
                RayfireMan.Log (RFLog.cmb_dbgn + name + RFLog.cmb_index, gameObject);
